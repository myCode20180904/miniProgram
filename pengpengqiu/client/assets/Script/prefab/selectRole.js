...
cc.Class({
    ...
    sel_view:function(){
        ...
        this.sel.getChildByName("name").getComponent(cc.Label).string = element.name;
        ...
        this.sel.getChildByName("descrich").getComponent(cc.RichText).string = attribute_str;
    ...
    Scroll_View:function(){
        ...
        let row = 0;
        let col = 0;
        this.scrollView.getComponent(cc.ScrollView).content.height = ((dataScript.userInfo.Scan.length-1)/3+1)*216
        for (let index = 0; index < dataScript.userInfo.Scan.length; index++) {
            ...
            item.setPosition(cc.v2(-W/2+194/2+row*194,-(216/2+col*216)));
            // item.setPosition(cc.v2(194+row*194,216+col*216));
