...
cc.Class({
    ...
    loadgame:function(){
        ...
        for (let index = 0; index < dataScript.gamedata.playerList.length; index++) {
            ...
            roleitem.getChildByName("body").scale = element.startScale+(element.level-1)*0.025;
    ...
    menu:function(event,customEventData){
        // console.info(event);
         ...
         if(customEventData=="pause"){
            ...
            dataScript.gamedata.playerList[0].speed = skillSpeed;
            this.player.getComponent("role")._FloatSlerp2.init(skillSpeed,0,0.4);
            //
            let resume_dt  = 0.4;
            ...
            let self_att = cc.sequence(cc.delayTime(resume_dt),cc.callFunc(function(obj){
                ...
                this.player.getComponent("role")._FloatSlerp2.init(rolebody.getComponent(cc.RigidBody).linearVelocity.mag(),info.startSpeed,0.4);
            },this))
    ...
    fuhuo:function(){
        ...
        this.player.getChildByName("body").scale = element.startScale+(element.level-1)**0.025;
        let key = `loadres/role/role${element.scan}`
        ...
        var that = this;
        this.player.runAction(cc.sequence(cc.fadeIn(0.5),cc.callFunc(function(){
            //物理属性
    ...
    onTouchMove:function(event){
        ...
        if(rate>1){
            ...
            this.control.getChildByName("center").setPosition(cc.v2(__dir.x*60/1,__dir.y*60/1));
            if(angle!=NaN){
