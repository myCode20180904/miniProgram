...
var setSafeMap = function(map){
    safeMap = map;
}
// startPos:起始位置
// endpos:目标位置
// out:输出结果
var findRote = function(startPos,endPos){
    ...
    // console.info("开始寻路");
    let count = 0;
    let allG = 0;//allG是从开始点start到当前方块的移动量
...
var setOpen = function(open,pos,except_map){
    ...
    for (let index = 0; index < arr.length; index++) {
        ...
        //安全区域的点优先选择
        if(safeMap.has(dataScript.gamedata.tomapkey(element))){
            open.push(element);    
        }else if(mapArr.has(dataScript.gamedata.tomapkey(element))){
            open.push(element);
        }
