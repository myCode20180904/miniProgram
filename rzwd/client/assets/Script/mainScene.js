...
cc.Class({
       ...
       // console.info(event);
        ...
        if(customEventData=="daysign"){
            ...
            if(dataScript.common.GameClubBn){
                dataScript.common.GameClubBn.hide();
            }
        ...
        }else if(customEventData=="startGame2"){
            console.info(cc.sys.localStorage.getItem('isNewPlayer'));
            if(cc.sys.localStorage.getItem('isNewPlayer')==1){
                dataScript.gamedata.startGame2();
                if(dataScript.common.GameClubBn){
                    dataScript.common.GameClubBn.hide();
                }
            }else{
    ...
    popWindows:function(){
         //今天未签到
         if(dataScript.userInfo.todaySign==0){
            this.openPage(null,"daysign");
        }

        //
        
        //游戏圈
        dataScript.common.GameClubBn = plat.createGameClubButton();
    ...
    refreshLevel:function(){
        let level = this.node.getChildByName("menuNode").getChildByName("startGame").getChildByName("level");
        let lv1 = level.getComponent(cc.Label);
        ...
        lv1.string = `${dataScript.gamedata.map_index+1}层`;
    ...
    refreshSelectUI:function(){
        ...
        let pageModel = this.select.getChildByName("pageModel");
        ...
        if(pageNum<dataScript.userInfo.roleScan.length){
            for (let index = 0; index <dataScript.userInfo.roleScan.length-pageNum; index++) {
                let newPage = cc.instantiate(pageModel);
                newPage.active = true;
